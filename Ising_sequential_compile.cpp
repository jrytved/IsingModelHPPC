...
std::vector<int> create_lattice(std::mt19937& generator){
    std::vector<int> lattice(K);
...
void evolve_lattice(std::mt19937& generator, std::vector<int>& white, std::vector<int>& black){
    // update all white
    for (size_t i = 0; i < N; ++i){
        for (size_t j = 0; j < M; ++j){
            ...
            size_t downI  = (I+1)   % N;
            // 
            int left_black  = black[I*M     + (leftJ  - 1 + (I%2))/2];
            int right_black = black[I*M     + (rightJ - 1 + (I%2))/2];
            int up_black    = black[upI*M   + (J - 1 + (upI  %2))/2];
            int down_black  = black[downI*M + (J - 1 + (downI%2))/2];
            int spin        = white[i*M     + j];
            // energy difference
    ...
    for (size_t i = 0; i < N; ++i){
        for (size_t j = 0; j < M; ++j){
            ...
            size_t downI  = (I+1)   % N;
            //
            int left_white  = white[I*M     + (leftJ  - (I%2))/2];
            int right_white = white[I*M     + (rightJ - (I%2))/2];
            int up_white    = white[upI*M   + (J - (upI  %2))/2];
            int down_white  = white[downI*M + (J - (downI%2))/2];
            int spin        = black[i*M     + j];
            // energy difference
...
void join_arrays(const std::vector<int>& white, const std::vector<int>& black, std::vector<int>& global){
    // implemented without conditionals
    for(size_t i=0; i < N; i++){
        for(size_t j=0; j < M; j++){
            global[i*N + (2*j + (i%2))]     = white[i*M + j];
            global[i*N + (2*j + 1 - (i%2))] = black[i*M + j];
        }
    }
}

void write_to_file(const std::vector<int> global){
...
void calculate_quantities(const std::vector<int>& global, double& magnetization, double& energy){
    magnetization = 0.0;
    energy = 0.0;
    
    for (size_t i = 0; i < N; ++i){
        for (size_t j = 0; j < N; ++j){
            magnetization += global[i*N + j];
            energy += -global[i*N + j] * (global[((i+1)%N)*N + j] + global[((i-1+N)%N)*N + j]
                                        + global[i*N + (j+1)%N] + global[i*N + (j-1+N)%N] + H);
        }
    }
    magnetization /= N*N;
...
int main(int argc, char* argv[]){
    ...
    std::vector<int> white = create_lattice(generator); 
    // create black lattice
    std::vector<int> black = create_lattice(generator);
    // create global lattice
    std::vector<int> global (2*K); // M is defined as N/2 in SystemConfig
